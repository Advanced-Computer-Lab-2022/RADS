...
const AdminProblems = (props) => {
  ...
  useEffect(() => {
    const fetchRequest = async () => {
      ...
      if (response.ok) {
        if (
          (!json.instructorId || json.instructorId === undefined) &&
          (!json.corpTraineeId || json.corpTraineeId === undefined)
        ) {
        ...
        if (
          (!json.traineeId || json.traineeId === undefined) &&
          (!json.corpTraineeId || json.corpTraineeId === undefined)
        ) {
        ...
        {isTrainee && !isInstructor && !isCorpTrainee ? (
          <Box>
            ...
            <Box>
              ...
              <p>
                {trainee.firstName}'s Comments for the problem:
                {request.traineeComments &&
                  request.traineeComments.map((comment, index1) => (
                    <p>
                      Comment {index1 + 1}: {comment.traineeComment}
                    </p>
                  ))}
              </p>
                ...
                <fieldset>
                  <label>
                    <input
                      ...
                      value="resolved"
                      name="resolved"
                      checked={checkedOption === "resolved"}
                      onChange={(e) => {
        ...
        {!isTrainee && isInstructor && !isCorpTrainee ? (
          <Box>
            ...
            <Box>
              ...
              <p>Course Name: {course.courseTitle} </p>
              <p>
                {instructor.firstName}'s Comments for the problem:
                {request.instructorComments &&
                  request.instructorComments.map((comment, index2) => (
                    <p>
                      Comment {index2 + 1}: {comment.instructorComment}
                    </p>
                  ))}
              </p>
              <p>Request Type: {request.requestType} Request</p>
      ...
      <Box>
        {!isTrainee && !isInstructor && isCorpTrainee ? (
          <Box>
            ...
            <Box>
              <p>
                Corporate Trainee Name: {corpTrainee.firstName}{" "}
                {corpTrainee.lastName}
              </p>
              <p>Course Name: {course.courseTitle} </p>
              <p>
                {corpTrainee.firstName}'s Comments for the problem:
                {request.corpTraineeComments &&
                  request.corpTraineeComments.map((comment, index3) => (
                    <p>
                      Comment {index3 + 1}: {comment.corpTraineeComment}
                    </p>
                  ))}
              </p>
              <p>Request Type: {request.requestType} Request</p>
