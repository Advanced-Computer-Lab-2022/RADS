  ...
  return (
    <Box>
      {status && status === true ? (
        <Box>
          <p>
            <strong>Already Solved the course exercises</strong>
          </p>
          <p>
            <strong>
              Exercises Grade was: {Math.ceil(oldExercisesGrade * 2)}%
            </strong>
          </p>
        </Box>
      ) : (
        <Box>
                    ...
                    <fieldset id={exercise._id}>
                      ...
                      <label>
                        <input
                          ...
                          name={exercise.firstChoice}
                          checked={choices[index] === exercise.firstChoice}
                          onChange={(e) => {
                            handleChoice(e, index);
                          }}
                        />{" "}
                        {exercise.firstChoice}
                      </label>
                      <label>
                        <input
                          id={`second${index}`}
                          ...
                          name={exercise.secondChoice}
                          checked={choices[index] === exercise.secondChoice}
                          onChange={(e) => {
                            handleChoice(e, index);
                          }}
                        />
                        {exercise.secondChoice}
                      </label>
                      <label>
                        <input
                          id={`third${index}`}
                          ...
                          name={exercise.thirdChoice}
                          checked={choices[index] === exercise.thirdChoice}
                          onChange={(e) => {
                            handleChoice(e, index);
                          }}
                        />
                        {exercise.thirdChoice}
                      </label>
                      <label>
                        <input
                          id={`forth${index}`}
                          ...
                          name={exercise.fourthChoice}
                          checked={choices[index] === exercise.fourthChoice}
                          onChange={(e) => {
                            handleChoice(e, index);
                          }}
                        />
                        {exercise.fourthChoice}
                      </label>
                    </fieldset>
                  </Box>
                ))}
              {showButton && (
                <Button variant="contained" id="submit-solve" type="submit">
                  Submit
                </Button>
              )}
            </form>
          ...
          <Box className="solution-form">
            {solved &&
              grades &&
              grades.map((grade, index2) => (
                <p>
                  Q{index2 + 1}: {grade} out of 1
                </p>
              ))}
            {solved &&
              corrects &&
              corrects.map((correct, index3) => (
                <p>
                  A{index3 + 1}: {correct}
                </p>
              ))}
            <p>
              <strong>
                Exercises Grade: {Math.ceil(exercisesGradeFinal * 2)}%
              </strong>
            </p>
          </Box>
        </Box>
