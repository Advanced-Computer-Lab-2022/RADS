  ...
  const ShowGrades = async () => {
    ...
    for (let i = 0; i < choices.length; i++) {
      if (choices[i] === examExercises[i].answer) {
        ...
        array2[
          i
        ] = `Incorrect! , correct solution is ${examExercises[i].answer}`;
      }
    }
    setSolved(true);
    setGrades(array1);
  ...
  return (
    <Box>
      {status && status === true ? (
        <Box>
          <p>
            <strong>Already Solved the exam</strong>
          </p>
          <p>
            <strong>Exam Grade was: {Math.ceil(oldExamGrade * 2)}%</strong>
          </p>
        </Box>
      ) : (
        <Box>
          ...
          <Box className="solution-form">
            {solved &&
              ...
              grades.map((grade, index) => (
                <p>
                  Q{index + 1} Grade: {grade} out of 1
                </p>
              ))}
            {solved &&
              corrects &&
